...
| 0:33 | เปิดแท็บ `https://jenosize-crm-ai.vercel.app` | "ระบบ deploy จริงอยู่บน Vercel เชื่อมกับฐานข้อมูล Postgres จริงบน Neon ไม่ใช่ mock data ครับ/ค่ะ ข้อมูล seed มีจริง 2,000 contacts และ 300 leads ตามโจทย์" |
...
| เวลา | Action หน้าจอ | คำพูด |
|---|---|---|
| 0:45 | อยู่หน้า `/login` กรอก `rep1@jenosize.demo` / `Passw0rd!` กด Sign in | "ลอง login ด้วยบัญชี Sales Rep คนหนึ่งก่อนนะครับ/ค่ะ" |
...
| 1:55 | ชี้ lead ใหม่ที่เพิ่งขึ้น (ชื่อ "LINE User ...") คลิกเข้าไปดู | "นี่ครับ/ค่ะ Lead ใหม่ที่มาจาก LINE โดยอัตโนมัติ พร้อมข้อความที่เพิ่งส่งเข้ามาอยู่ใน timeline แล้ว" |
...
| 3:12 | เปิด terminal รันคำสั่ง (หรือโชว์ผลลัพธ์ที่รันไว้ล่วงหน้า) `npm run test:e2e` ให้เห็นบรรทัด "14 passed" | "รันได้ผ่านครบ 14 เทสต์ ทั้งกับ local และยืนยันซ้ำกับ production จริงบน Vercel ด้วย" |
...
| 4:05 | กลับมาหน้า `/leads` ของเว็บที่ deploy จริง | "สรุปคือ ระบบนี้ใช้งานได้จริงครบทั้ง 3 Part ตามโจทย์ — Working CRM, AI Copilot ที่แยก draft ออกจาก confirmed write, และ LINE OA integration ของจริง พร้อม audit trail และ automated test ครบ" |
